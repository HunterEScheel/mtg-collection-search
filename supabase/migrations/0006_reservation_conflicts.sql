-- Reservation conflict detection: validate the whole selection under row
-- locks BEFORE creating anything. If any selected card has been taken (row
-- gone) or partially taken (fewer copies left than requested) since the buyer
-- loaded the page, nothing is written and the conflicts come back as data:
--   { "ok": false, "conflicts": [{ "id", "name", "requested", "available" }] }
-- On success:
--   { "ok": true, "location_id", "share_id", "name", "moved" }
