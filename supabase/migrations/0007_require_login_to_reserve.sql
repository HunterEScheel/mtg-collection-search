-- Reserving now requires a signed-in user: reserved_by is always set, so
-- every reservation can be transferred to its buyer. Anonymous visitors can
-- still VIEW shared locations (get_shared_location keeps its anon grant).
...
revoke execute on function public.reserve_cards(uuid, jsonb, text) from anon;
