...
export function DeleteLocationDialog({ location, cardCount, onDeleted, onClose }: Props) {
  ...
  async function confirmDelete() {
    setBusy(true);
    setError(null);
    const { error } = await supabase.from('collections').delete().eq('id', location.id);
    if (error) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      ...
      >
        <h2 className="text-lg font-semibold text-red-400">Delete location?</h2>
        <p className="text-sm text-zinc-300">
          This permanently deletes <span className="font-semibold">“{location.name}”</span>
          {cardCount > 0 && <> and its <span className="font-semibold">{cardCount}</span> card rows</>}.
          This cannot be undone. Your original CSV export is unaffected — you can re-import it any time.
        </p>
